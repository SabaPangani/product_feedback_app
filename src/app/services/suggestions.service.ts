...
import {
  FeedbackByStatus,
  FeedbackRequest,
} from '../data-model/feedback-model';
...
export class SuggestionsService {
  ...
  sortData(
    sortFn: (a: FeedbackRequest, b: FeedbackRequest) => number
  ...
  sortFeedbacks(sortStyle: string): Observable<FeedbackRequest[]> {
    let sortFn: (a: FeedbackRequest, b: FeedbackRequest) => number = (a, b) =>
      0;
      ...
      map((response) => {
        ...
        if (cat === 'All') {
          ...
          return data.filter(
            (item) =>
              item.category &&
              item.category.toString().toLowerCase() === cat.toLowerCase()
          );
        }
      })
    );
